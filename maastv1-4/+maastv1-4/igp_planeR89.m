function [sig2_igp, mu_igp, sig2_upm, r, rcm, idx, delay, chi2] ...
                            = igp_planeR89(xyz_igp, igp_en_hat, igp_cornerden, ...
...
global GIVE2_N_MIN GIVE2_N_PTS GIVE2_RMAX GIVE2_RMIN GIVE2_SIG2_DECORR ...
       GIVE2_CHI2_LOWER GIVE2_CHI2_LWR_UPM GIVE2_CHI2_NOM GIVE2_RIRREG2_FLOOR
global TRUTH_FLAG
...
%    check minimum radius
  if(n_fit >= GIVE2_N_PTS)
    ...
    r=sqrt(temp(GIVE2_N_PTS));
    idx=idx(fit_idx(1:GIVE2_N_PTS));
    n_fit=GIVE2_N_PTS;
  end
end
...
W=inv(M(idx,idx)+GIVE2_SIG2_DECORR*eye(length(idx)));
...
if TRUTH_FLAG
    ...
    Wo=inv(GIVE2_SIG2_DECORR*eye(length(idx)));
    covo=inv(G'*Wo*G);
    ao=covo*G'*Wo*Ivpp(idx);
    chi2_raw=Ivpp(idx)'*Wo*(Ivpp(idx)-G*ao);
    Rnoise = 1;
else
    Rnoise = prod(1+diag(M(idx,idx))/GIVE2_SIG2_DECORR)^(1/n_fit);
    chi2_raw = GIVE2_CHI2_NOM(n_fit-3)/Rnoise;
end

chi2 = chi2_raw*Rnoise;

Rirreg2=chi2/GIVE2_CHI2_LOWER(n_fit-3);
Rirreg2=max(GIVE2_RIRREG2_FLOOR,Rirreg2);
cov = cov0*G'*W*(Rirreg2*GIVE2_SIG2_DECORR*eye(length(idx)) + ...
                  M(idx,idx))*W*G*cov0;

sig2_igp = cov(1,1) + Rirreg2*GIVE2_SIG2_DECORR;

%calculate antenna bias term
mu_igp = [1 0 0]*abs(cov0*G'*W)*mu_me(idx);

%calculate UPM GIVE value for GEO UDRE
Rirreg_realistic2 = chi2/GIVE2_CHI2_LWR_UPM(n_fit-3);
Rirreg_realistic2 = max(GIVE2_RIRREG2_FLOOR,Rirreg_realistic2);
cov_realistic = cov0*G'*W*(Rirreg_realistic2*GIVE2_SIG2_DECORR*eye(length(idx)) ...
                   + M(idx,idx))*W*G*cov0;
sig2_upm = cov_realistic(1,1) + Rirreg_realistic2*GIVE2_SIG2_DECORR;

%calculate Relative Centroid Metric
We=e(idx)./(diag(M(idx,idx)) + GIVE2_SIG2_DECORR);
