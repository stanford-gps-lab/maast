...
if (svdata.mt1_time >= (time - MOPS_MT1_PATIMEOUT)) && ...
    ...
    fc_timeout = MOPS_MT7_FCTIMEOUT(svdata.mt7_ai+1);
    ...
    idx = mod(svdata.mt2_fc_iodf(:,1) - svdata.mt2_fc_iodf(:,2),3) ~= 1 & ...
           ~isnan(dtrrc);
    if any(idx)
        eps_rrc_deg(idx) = (MOPS_MT7_AI(svdata.mt7_ai(idx)+1).*fc_timeout(idx)/4 + ...
    ...
    idx = (dtfc > fc_timeout  | tudrei > MOPS_UDRE_PATIMEOUT  | ...
    ...
    idx = tudrei <= fc_timeout & (svdata.mt2_udrei == MOPS_UDREI_DNU ...
