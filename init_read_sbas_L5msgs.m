...
nmeo = size(alm_param,1);
nsat = size(satdata, 1);
ngeo = nsat - nmeo;
geoprns = satdata((nmeo+1):end, COL_SAT_PRN);
...
elseif n_channels < ngeo
    ...
    [~, jdx] = setdiff(satdata(nmeo+(1:ngeo),COL_SAT_PRN), geoprns(idx));
    satdata(nmeo+jdx,:) = [];
    ngeo = length(gdx);
    nsat = nmeo + ngeo;
end    
...
satdata((nmeo+1):end, COL_SAT_XYZ) = NaN;
...
for gdx = n_channels:-1:1
    ...
    if gdx == gprime
        ...
        if svdata(gdx).mt31(1).time >= tstart - L5MOPS_MT31_PATIMEOUT
            
            prns = svdata(gdx).prns;
            %check that the gps mask matches the gps almanac
            while svdata(gdx).mt31(1).nmeo ~= nmeo || ...
                    ~isequal(prns(1:nmeo), satdata(1:nmeo,COL_SAT_PRN))
                if svdata(gdx).mt31(1).nmeo > nmeo 
                    [missing_prns, idx] = setdiff(prns(1:svdata(gdx).mt31(1).nmeo), ...
                                             satdata(1:nmeo,COL_SAT_PRN));
                    ...
                    [~, idx] = setdiff(satdata(1:nmeo,COL_SAT_PRN), ...
                                             prns(1:svdata(gdx).mt31(1).nmeo));
                    satdata(idx,:) = [];
                    alm_param(idx,:) = []; 
                    nmeo = nmeo - length(idx);
