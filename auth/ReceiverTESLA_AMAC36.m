classdef ReceiverTESLA_AMAC36 < ReceiverTESLA
    methods

        function obj = ReceiverTESLA_AMAC36(varargin)
            obj@ReceiverTESLA(varargin{:});
            obj.mac_signing_function = @(x, y)HashingWrappers.padded_hmac_sha_256(x, y, 5);
            obj.mt50_decode = @(message, time, prn)MT50_AMAC36.decode(message, time, prn);
            obj.hmac_size = 5;
            obj.include_crc = false;
        end

        function verify_hmac_block(obj, MT50, hashpoint, time)
            hmacs = uint8(zeros(5, 5));
            flag = [1, 1, 1, 1, 1];
            hi = 0;
            hmac_count = 0;
            ...
            elseif hmac_count < 5
                % Recover lost HMACS
                hmacs = DataRecovery.recover_data_36(hmacs, [MT50.get_datarec(1), MT50.get_datarec(2)], flag);
            end

            % Aggregate HMACs
            amac = CreateAMAC.aggregate(36, time, MT50.hash_point, obj.prn, hmacs);
        ...
        function output = check_if_dummy_mt50(obj, message)
            ...
            dummy_mt50 = MT50_AMAC36(uint8(zeros(obj.hmac_size, 1)), ...
