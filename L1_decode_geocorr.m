...
for mdx = 1:ngeos
    ...
    if svdata(mdx).mt9(1).time >= (time - MOPS_MT9_PATIMEOUT)

        %find the geo position
        tmt0 = time - svdata(mdx).mt9(1).t0;
        tmt0 = tmt0 - svdata(mdx).mt9(1).af0 - svdata(mdx).mt9(1).af1*tmt0;
        svdata(mdx).geo_xyzb(mdx,1:3) = svdata(mdx).mt9(1).xyz + ...
                                    svdata(mdx).mt9(1).xyz_dot*tmt0 + ...
                                    svdata(mdx).mt9(1).xyz_dot_dot*(tmt0^2)/2;
